...
import fetchNovelsFilteredData from "./NovelsFilteredData";

function NovelsListFilters(props) {
  const {
    searchParams,
    setPagedata,
    router,
  ...
  const handleSubmit = async (data) => {
    const filteredData = await fetchNovelsFilteredData(
      data,
      searchParams,
      router.replace
    );
              ...
              {(formProps) => (
                <Form className="mb-12">
                  <Stack spacing={1} alignSelf={"center"}>
                    ...
                    <Stack spacing={2} direction={"row"} sx={{ width: "46%" }}>
                      <FiltersControls control="order" />
                    </Stack>
...
export default NovelsListFilters;
