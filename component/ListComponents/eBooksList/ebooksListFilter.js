...
import fetchebooksFilteredData from "./ebooksFilteredData";
...
function ebooksListFilter(props) {
  const {
    searchParams,
    setPagedata,
    router,
  ...
  const initialValues = {
    ...
    departments: searchParams?.get("search")?.toString() || "",
    order: searchParams?.get("order")?.toString() || "",
    orderBy: searchParams?.get("orderBy")?.toString() || "",
    availability: searchParams?.get("availability")?.toString() || "",
  ...
  const handleSubmit = async (data) => {
    const filteredData = await fetchebooksFilteredData(
      data,
      searchParams,
      router.replace
    );
...
export default ebooksListFilter;
