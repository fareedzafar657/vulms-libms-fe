...
function issuance({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      Swal.fire("Successful!", "You returned a Novel!", "success");
      router.push(`/novels/detail/${asset_id}`);
    } catch (error) {
      Swal.fire(
        "you failed to return a Novel!",
      ...
      <Formik initialValues={initialValues} onSubmit={handleSubmit}>
        <div className="bg-white border-4 rounded-lg shadow relative m-10">
          ...
          <div className="flex justify-center p-5  border-b rounded-t">
            <h3 className="text-3xl  text-sky-700 font-bold">Novel Return</h3>
          </div>
          <Form className="p-6 space-y-6">
            ...
            <Stack spacing={2} direction={"row"} className="flex justify-end">
              <Button
                ...
                onClick={() => router.push(`/novels/detail/${asset_id}`)}
              >
                Cancel
              </Button>
              <Button
