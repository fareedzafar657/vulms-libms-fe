...
function reIssuance({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Swal.fire("Successful!", "You re-issued a Novel!", "success");
      router.push(`/novels/detail/${issued_id}`);
    } catch (error) {
      Swal.fire(
        "you failed to re-issued a Novel!",
          ...
          <Form className="p-6 space-y-6">
            ...
            <Stack spacing={2} direction={"row"} className="flex justify-end">
              <Button
                ...
                onClick={() => router.push(`/novels/detail/${issued_id}`)}
              >
                Cancel
              </Button>
              <Button
