...
import fetchLanguageFilteredData from "../../../component/ListComponents/LanguagesList/LanguageFilteredData";
import LanguageFilter from "../../../component/ListComponents/LanguagesList/LanguageListFilter";
...
export async function getServerSideProps(context) {
  ...
  try {
    const res = await axios.get(`/languages/pagedata?${params.toString()}`, {
      headers: {
...
function Language({ records }) {
  ...
  const [currentPage, setCurrentPage] = useState(pagedata.meta.data);
    ...
    <Layoutt>
      <div className="relative bg-white border border-gray-300 pt-6 pb-6 shadow-xl 3xl:w-[1920px] p-4 mx-auto min-w-full rounded-md overflow-x-auto">
        ...
        {/* <div className="flex justify-end md:mr-28 p-4">
            <Link href="/language/createlanguage" underline="none">
                  ...
                  {pagedata.data.map((languages) => {
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center">{languages.name}</TableCell>

                        <TableCell align="left">
                          <Stack direction={"row"}>
                            <EditButton
                              link={`/language/${languages.id}`}
                              router={router}
                            />

                            <DeleteButton
                              link={`/languages/delete/${languages.id}`}
                              router={router}
                            />
                          </Stack>
                        </TableCell>
            ...
            <div className="flex  md:justify-between p-4">
              <Stack spacing={2} className="md:mr-34">
                <FiltersControls
                  control="take"
                  setPagedata={setPagedata}
                  filteringData={fetchLanguageFilteredData}
