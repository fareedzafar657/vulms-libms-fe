...
export async function getServerSideProps(context) {
  ...
  const response = await axios.get(`/ebooks/update/${params.updateebooks}`, {
    headers: {
...
function updateEbooks({
  ...
  const initialValues = {
    ...
    distributerId: bookRecord.distributer
      ? `${bookRecord.distributer.id}`
      : null,
    ...
    currencyId: bookRecord.currency ? `${bookRecord.currency.id}` : null,
  ...
  const handleSubmit = async (values, bookId) => {
    try {
      ...
      Swal.fire("Successful!", "You updated a e-book!", "success");
      router.push(`/ebooks/list`);
    } catch (error) {
      // Swal.fire("failed!", "You failed to update a e-book!", "");
      // console.error("Failed:", error.response.data.message);
      Swal.fire(
        "you failed to updated a e-book!",
