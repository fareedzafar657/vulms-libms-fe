...
function createJournals() {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await axiosAuth.post("/journals/create", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      Swal.fire("Successful!", "You added a Journals!", "success");
      router.push(`/journals/list`);
    } catch (error) {
      Swal.fire(
        "you failed to add Journal!",
        ...
        {(formProps) => (
          <div className="bg-white   border-4 rounded-lg shadow relative m-10">
            <div className="flex justify-center p-5  border-b rounded-t">
              <h3 className="text-3xl  text-sky-700 font-bold">Add Journals</h3>
            </div>
            ...
            <Form className="p-6 space-y-6">
              ...
              {showDonationDetails && (
                <>
                  ...
                  <Stack spacing={2} direction={"row"} sx={{ width: "600px" }}>
                    <FieldsControls
                      id="outlined-basic"
                      variant="outlined"
                      control="text"
