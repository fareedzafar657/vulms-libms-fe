...
function issuance({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Swal.fire("Successful!", "You re-issued a Journal!", "success");
      router.push(`/journals/detail/${issued_id}`);
      // router.reload();
    } catch (error) {
      Swal.fire(
        "you failed to re-issued a Journal!",
          ...
          <Form className="p-6 space-y-6">
            ...
            <Stack spacing={2} direction={"row"} className="flex justify-end">
              <Button
                ...
                onClick={() => router.push(`/journals/detail/${issued_id}`)}
              >
                Cancel
              </Button>
              <Button
